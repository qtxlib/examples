...
bool kbhit; // we have to emulate getch() and kbhit() without conio.h
int getch; // to emulate getch
...
int main()
    ...
    txKeyEvent(keyHandler); // to emulate kbhit & getch()
    txTextCursor (false);
    ...
    AppearText  (centerX, sizeY*4/5, "\"Hello, world! \" :)",                                TX_BLACK,  TX_LIGHTGREEN, 5000, 100);
...
void JumpMan (int x, int y, int sizeX, int sizeY, double jump,
    ...
    for (int i = 0, kbhit = false; i < jumps && !kbhit; i++)
        {
...
void MoveMan (int fromX, int fromY, int toX, int toY,
    ...
    for (int i = 0, kbhit = false; i <= steps && !kbhit; i++)
        {
...
void DrawFrame (int sizeX, int sizeY, int size, COLORREF color)
    ...
    txSetFillColor(TX_BLACK); txRectangle(0, 0, 800, 600); // we need the screen to be blacken for full compatibility
    }

//----------------------------------------------------------------------------

int kbget() // replace function calls with variables
    {
    int ch = getch;
